...
class RegressionData:
    def __init__(self, t, y, t_surface):
...
class SklearnModel(BaseModel):
    def __init__(self, name, priority, predictor, param_grid, feat_key):
    ...
    def predict_eval(self, data_eval):
        return self.best_predictor.predict(data_eval.features[self.feat_key])

    def predict_surface(self, full_data):
        return self.best_predictor.predict(full_data.features[self.feat_key + '_s'])
...
class RBFNetworkModel(BaseModel):
    ...
    def predict_eval(self, data_eval):
        G = self._rbf_basis(data_eval.features['t'])
        return self.lr.predict(G)

    def predict_surface(self, full_data):
        G = self._rbf_basis(full_data.features['t_s'])
        return self.lr.predict(G)
...
class ModelRunner:
    ...
    def _build_models(self):
        models = [
            SklearnModel("Inverse Regression", 1, Pipeline([('inv', FunctionTransformer(lambda x: 1.0/(x+1e-8))), ('lr', LinearRegression())]), None, 't'),
            SklearnModel("Linear Regression", 2, LinearRegression(), None, 't'),
            FGLSModel("Weighted Linear Regression", 3),
            SklearnModel("Robust Regression (Huber)", 4, HuberRegressor(), None, 't'),
            SklearnModel("Theil–Sen Regression", 5, TheilSenRegressor(random_state=42), None, 't'),
            SklearnModel("RANSAC Regression", 6, RANSACRegressor(random_state=42), None, 't'),
            SklearnModel("Ridge Regression", 7, Ridge(random_state=42), {'alpha': [0.1, 1.0]}, 't'),
            SklearnModel("Lasso Regression", 8, Lasso(random_state=42), {'alpha': [0.1, 1.0]}, 't'),
            SklearnModel("Elastic Net Regression", 9, ElasticNet(random_state=42), {'alpha': [0.1, 1.0], 'l1_ratio': [0.5]}, 't'),
            SklearnModel("Bayesian Linear Regression", 10, BayesianRidge(), None, 't'),
            SklearnModel("Bayesian Polynomial Regression", 11, Pipeline([('poly', PolynomialFeatures(degree=3)), ('br', BayesianRidge())]), None, 't'),
            ODRModel("Orthogonal Distance Regression (ODR)", 12),
            TLSModel("Total Least Squares (TLS)", 13, feat_key='t'),
            DemingModel("Deming Regression", 14),
            SklearnModel("2nd Degree Polynomial Regression", 15, Pipeline([('poly', PolynomialFeatures(degree=2)), ('lr', LinearRegression())]), None, 't'),
            SklearnModel("3rd Degree Polynomial Regression", 16, Pipeline([('poly', PolynomialFeatures(degree=3)), ('lr', LinearRegression())]), None, 't'),
            SklearnModel("4th Degree Polynomial Regression", 17, Pipeline([('poly', PolynomialFeatures(degree=4)), ('lr', LinearRegression())]), None, 't'),
            CurveModelSource1("Exponential Regression", 18, lambda x, a, b, c: a * np.exp(np.clip(b * x, -50, 50)) + c, lambda d: [1.0, 0.01, np.min(d.y)]),
            CurveModelSource1("Logarithmic Regression", 19, lambda x, a, b: a * np.log(np.clip(x, 1e-8, None)) + b, lambda d: [1.0, np.mean(d.y)]),
            CurveModelSource1("Power Regression", 20, lambda x, a, b, c: a * np.power(np.clip(x, 1e-8, None), b) + c, lambda d: [1.0, 1.0, 0.0], lambda d: ([-np.inf, -5, -np.inf], [np.inf, 5, np.inf])),
            CurveModelSource1("Gaussian Regression", 21, lambda x, a, b, c, d_c: a * np.exp(-(x - b)**2 / (2 * c**2 + 1e-8)) + d_c, lambda d: [np.max(d.y)-np.min(d.y), d.t[np.argmax(d.y)], max(np.std(d.t), 1e-5), np.min(d.y)], lambda d: ([0, -np.inf, 1e-5, -np.inf], [np.inf, np.inf, np.inf, np.inf])),
            CurveModelSource1("Rational Function Regression", 22, lambda x, a, b, c, d_p: (a * x + b) / (c * x + d_p + 1e-8), lambda d: [1.0, 1.0, 1.0, 1.0]),
            CurveModelSource1("Hyperbolic Regression", 23, lambda x, a, b, c: a + b / (x + c + 1e-8), lambda d: [np.mean(d.y), 1.0, 1.0]),
            SklearnModel("Reciprocal Polynomial Regression", 24, Pipeline([('inv', FunctionTransformer(lambda x: 1.0/(x+1e-8))), ('poly', PolynomialFeatures(degree=2)), ('lr', LinearRegression())]), None, 't'),
            CurveModelSource1("Padé Approximation Regression", 25, lambda x, a0, a1, b1: (a0 + a1 * x) / (1.0 + b1 * x + 1e-8), lambda d: [np.mean(d.y), 0.1, 0.1]),
            PiecewiseLinearModelSource1("Piecewise Regression", 26, poly_deg=0),
            PiecewiseLinearModelSource1("Piecewise Linear Regression", 27, poly_deg=1),
            PiecewiseLinearModelSource1("Piecewise Polynomial Regression", 28, poly_deg=2),
            LOESSModelSource1("LOESS (LOWESS) Regression", 29, deg=1),
            LOESSModelSource1("Local Polynomial Regression", 30, deg=2),
            KernelRegressionModel("Kernel Regression (Nadaraya–Watson)", 31),
            GAMModel("Generalized Additive Model (GAM)", 32),
            SklearnModel("Quantile Regression", 33, QuantileRegressor(alpha=0.0, solver='highs'), None, 't'),
            SplineModel("Cubic Spline Regression", 34),
            NaturalCubicSplineModel("Natural Cubic Spline Regression", 35),
            BSplineRegressionModel("B-Spline Regression", 36),
            SmoothingSplineModel("Smoothing Spline Regression", 37),
            PSplineModel("Penalized Spline Regression (P-Spline)", 38),
            TPSModel("Thin Plate Spline Regression", 39),
            SklearnModel("2nd Degree Fourier Series", 40, Pipeline([('fourier', FunctionTransformer(_fourier_basis)), ('lr', LinearRegression())]), None, 't'),
            CurveModelSource1("Sinusoidal Regression", 41, lambda x, a, w, phi, c: a * np.sin(w * x + phi) + c, lambda d: [np.std(d.y), 2.0*np.pi/max(1e-5, np.max(d.t)-np.min(d.t)), 0.0, np.mean(d.y)]),
            MovingAverageRegressionModel("Moving Average Regression", 42),
            PureExponentialSmoothingModel("Exponential Smoothing", 43),
            HoltLinearTrendModel("Holt Linear Trend", 44),
            HoltWintersModel("Holt-Winters Method", 45),
            PureARModel("Autoregressive (AR) Model", 46),
            PureARMAModel("Autoregressive Moving Average (ARMA)", 47),
            ARIMAModel("Autoregressive Integrated Moving Average (ARIMA)", 48),
            SARIMAModel("Seasonal ARIMA (SARIMA)", 49),
            ComprehensiveETSModel("Error–Trend–Seasonality (ETS)", 50),
            DynamicLinearModel("Dynamic Linear Model (DLM)", 51),
            StateSpaceRegressionModel("State Space Regression", 52, feat_key='t'),
            KalmanFilterRegressionModel("Kalman Filter Regression", 53, feat_key='t'),
            SklearnModel("Support Vector Regression (SVR)", 54, SVR(), {'C': [1, 10], 'epsilon': [0.1]}, 't'),
            SklearnModel("Decision Tree Regression", 55, DecisionTreeRegressor(random_state=42), None, 't'),
            SklearnModel("Random Forest Regression", 56, RandomForestRegressor(random_state=42, n_jobs=1, n_estimators=50), None, 't'),
            SklearnModel("Extra Trees Regression", 57, ExtraTreesRegressor(random_state=42, n_jobs=1, n_estimators=50), None, 't'),
            SklearnModel("Gradient Boosting Regression", 58, GradientBoostingRegressor(random_state=42, n_estimators=50), None, 't'),
            SklearnModel("AdaBoost Regression", 59, AdaBoostRegressor(random_state=42, n_estimators=50), None, 't'),
            SklearnModel("Extreme Gradient Boosting (XGBoost)", 60, XGBRegressor(random_state=42, n_jobs=1, n_estimators=50), None, 't'),
            SklearnModel("Light Gradient Boosting Machine (LightGBM)", 61, LGBMRegressor(random_state=42, n_jobs=1, n_estimators=50, verbose=-1), None, 't'),
            SklearnModel("CatBoost Regression", 62, CatBoostRegressor(verbose=0, random_state=42, thread_count=1, iterations=50), None, 't'),
            SklearnModel("k-Nearest Neighbors Regression (kNN)", 63, KNeighborsRegressor(), {'n_neighbors': [3, 5]}, 't'),
            SklearnModel("Gaussian Process Regression (GPR)", 64, GaussianProcessRegressor(random_state=42), None, 't'),
            SklearnModel("Multi-Layer Perceptron (MLP)", 65, MLPRegressor(max_iter=500, random_state=42), None, 't'),
            RBFNetworkModel("Radial Basis Function Neural Network (RBF Network)", 66)
        ]
...
def get_best_fit(t, y, t_surface):
    data = RegressionData(t, y, t_surface)
