WARNING: To run the code, move the ModelPredictData.py file from the Requirements folder to the following path: C:\Users\user\AppData\Local\Programs\Python\Python311\Lib\site-packages\ModelPredictData.py.
...
This code uses 66 different models for Sensor Percentage when making predictions and selects the one with the highest accuracy rate for each.
...
If the sensor you are using is listed, browse to the 4D_Curve folder.
If the sensor you are using is listed but not in the 4D_Curve list, examine the ModelGases folder. (ModelGases --> ModelCurve.py)
...
-- NOTES --
Since the correction coefficient of unsupported sensors cannot be calculated, it is included in the "Models" folder instead of the "4D_Curve" folder.
