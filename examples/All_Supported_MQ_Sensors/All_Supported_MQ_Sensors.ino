// (1) The library also supports data science applications such as gradient 4D Curve Estimation from Python!:
// (1) https://github.com/abcdaaaaaaaaa/MQDataScience/tree/main/DataScience/4D_Curve
...
void loop() {
    ...
    for (byte i = 0; i < sensorModel->gasCount; i++) {
         ...
         * Therefore, after the final value is found, the result must be multiplied by ×50 to convert the value to ppm. */
          ...
          if (mode == "MQ306A" && i == 0) RsRscalValue = 0.976666;
          if (mode == "MQ307A" && i == 1) RsRscalValue = 0.999572;
          if (mode == "MQ309A" && i >= 2) RsRscalValue = 0.988237;
